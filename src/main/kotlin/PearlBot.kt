        ...
        suspend fun startBot() {
            bot(token) {
                ...
                commands("!") { // "." is the default, but is provided here anyway for example purposes
                    ...
                    {
                        reply(
                            this.author.username.toString() + "is an idiot." + "\n" + "What else is new."
                        )
                    }
                    command("judge")
                    {
                    ...
                    command("ping") {
                        ...
                        var latency =
                            ZonedDateTime.parse(message.sentAt).toInstant().toEpochMilli() - ZonedDateTime.parse(sentAt)
                    ...
                    command("pp") {
                        ...
                        this.usersMentioned.let {
                            ...
                            points?.let {
                                ...
                                val successful =
                                    PointsService.givePoints(this.author, recipient, this.guildId!!.toLong(), points)
                                when (successful) {
